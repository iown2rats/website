...
Signing in locally: sign-in is Google or Telegram (both OpenID Connect). With `AUTH_PROVIDER=dev` (the default outside production) "Continue with Google" opens a local stand-in for Google's account chooser at `/dev/google/authorize`, listing the seeded demo identities (`me@demo.thundi.dev` is the prototype's Ismail, `plus@demo.thundi.dev` the Plus user) or any typed email; "Continue with Telegram" opens the same stand-in in its Telegram shape (a username instead of an email, no email claim). Production needs `AUTH_PROVIDER=google` plus a Google OAuth client, and `TELEGRAM_CLIENT_ID` / `TELEGRAM_CLIENT_SECRET` from BotFather for the Telegram button. Photos are stored under `LOCAL_STORAGE_DIR` (`.storage`, git-ignored) and served through signed `/api/media` URLs. The dev provider and local storage are refused when `NODE_ENV=production`.
...
src/app/         Next.js routes: /, /auth/{google,telegram}/{start,callback}, /auth/{deleted,error,logout}, /onboarding/[stage], (app)/* incl. /chats/[conversationId], /community/[postId], /profile/{edit,preview}, /settings/*, /api/photos, /api/media, /api/community/posts, /dev/design-system
