...
Signing in locally: sign-in is Google or Telegram (both OpenID Connect). With `AUTH_PROVIDER=dev` (the default outside production) "Continue with Google" opens a local stand-in for Google's account chooser at `/dev/google/authorize`, listing the seeded demo identities (`me@demo.thundi.dev` is the prototype's Ismail, `plus@demo.thundi.dev` the Plus user) or any typed email; "Continue with Telegram" opens the same stand-in in its Telegram shape (a username instead of an email, no email claim). Email + password is the third method: with `EMAIL_PROVIDER=console` (the default outside production) the verification and reset links are printed to the dev-server log instead of being emailed, so the whole flow works locally. Production needs `AUTH_PROVIDER=google` plus a Google OAuth client, `TELEGRAM_CLIENT_ID` / `TELEGRAM_CLIENT_SECRET` from BotFather for the Telegram button, and a real mail provider (`EMAIL_PROVIDER=resend` with `RESEND_API_KEY` and `EMAIL_FROM`) before email sign-in appears at all. Photos are stored under `LOCAL_STORAGE_DIR` (`.storage`, git-ignored) and served through signed `/api/media` URLs. The dev provider and local storage are refused when `NODE_ENV=production`.
...
src/app/         Next.js routes: /, /auth/{google,telegram}/{start,callback}, /auth/{register,verify,verify-email,forgot-password,reset-password}, /auth/{deleted,error,logout}, /onboarding/[stage], (app)/* incl. /chats/[conversationId], /community/[postId], /profile/{edit,preview}, /settings/*, /api/photos, /api/media, /api/community/posts, /dev/design-system
