...
- **Delete account** requires recent authentication: `requestDeletionCode` sends a fresh OTP to the account's own phone through the same `requestOtp` rules and `SmsProvider` abstraction as sign-in (development: console provider with echo), and `deleteAccount` verifies it with `verifyOtpCode` (the sign-in check factored out so it consumes the challenge without creating a session) and refuses a code issued to any other phone. The UI is a two-step sheet (consequences, then code). Deletion runs in one transaction and **anonymises** rather than hard-deletes: photos (rows and files), interests, prompts, profile text, likes, passes, notifications, contact hashes, push subscriptions and sessions are removed; posts and comments are soft-deleted with counters recomputed; active matches become UNMATCHED and their conversations LOCKED; the User row becomes `status = DELETED`, `phoneE164 = deleted:<id>`, random `phoneHash`, no DOB or gender, so the number can register again as a new account. **Kept**: Report, Block, Message history (the other participant's record and evidence), AuditLog (`account.deleted`) and Subscription rows (billing records). Open policy: see §20 (retention period for anonymised rows and message history).
...
- Account deletion retention (Phase 9): deletion anonymises immediately but keeps the anonymised User row, message history, reports, blocks and subscription records indefinitely because no legal retention period has been decided. The owner (with legal advice) must set how long those records are kept before a purge job is written; no duration was invented.
