...
- Google OAuth client (Google Cloud project, OAuth consent screen, client id and secret) with `${APP_URL}/auth/google/callback` registered as an authorised redirect URI for every environment; set `AUTH_PROVIDER=google`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`. Until then only the development identity provider works, and it is refused in production.
