...
Phase 10 built the screen (`/likes`, `src/server/likes/likes-page.ts`): Free receives `count` and `placeholders` (blurhash + verified flag) and renders blurred tiles behind a lock card ("See who likes you · Plus feature · Upgrade to Plus" → Membership); Plus receives the standard discovery card DTO and can open a profile, like back (a mutual like shows the match screen) or pass. The Matches tab lists active matches with a way into each chat. Tested in `tests/integration/plus-enforcement.test.ts`: the Free payload contains no names, handles, ids or URLs.
...
- **Telegram sign-in goes live only after** migration `20260918160000_telegram_auth` is applied to the hosted database with owner approval (`docs/DEPLOYMENT.md` §3d) and the deployment is redeployed; the BotFather Login Widget and the two Vercel variables are already in place. Until then the Telegram routes 404 in production and the button is hidden.
