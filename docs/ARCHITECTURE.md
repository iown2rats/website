...
**Update 2026-09-24 — once means once.** Two defects in the above, both fixed without touching a single notification row. (1) The match-email throttle was a one-per-week `consumeRateLimit`, whose windows are epoch-aligned: a match emailed on a Wednesday became sendable again at Thursday 00:00 UTC, and the sweep (which keeps unread matches for three days) sent it twice. It is now `claimOnce`, a claim with no window; keys the old limit consumed count as claimed. (2) The digest counted every unread `LIKE_RECEIVED` row, so the same likes were re-announced daily for a week, and its number could disagree with Likes You. It now counts likes that are eligible by the shared Likes You rule (§12.5, `src/server/likes/eligibility.ts`), still unread in the feed, settled, and newer than the previous digest — at most one per 24 hours measured from the last send. The send time is logged as a `RateLimitBucket` row (`email:likes:digest:<member>`, `windowStart` = sent at); digests sent before the change are read as having gone at the end of their UTC day, the reading that can only err towards silence.

## 12.19 Plus promotion (2026-09-24)

Plus appears where a member already has a reason to want it, and every personal claim comes from the same server rule as the surface it describes. Nothing here changes an entitlement, a limit, the photo policy, the swipe engine or matching.

- **One count.** `countEligibleIncomingLikes` runs the same FROM/WHERE builder as `listEligibleIncomingLikes`, uncapped. Likes You, Membership ("4 people like you") and the Discover prompt all use it; zero means say nothing personal; Plus members are never sent it (`getLikesTeaser` returns null).
- **Free preview tiles** come only from an APPROVED photo, whatever `PHOTO_VISIBILITY_POLICY` says; with none, the plain placeholder.
- **Locked photos** say "N more photos": the count of locked entries the server sent (§12.18), so it cannot exceed what exists and never appears for Plus or matches.
- **Like limit** (Free): "You've used today's likes", the real reset time, that passing still works, "Get Plus". The Plus allowance comes from `PRODUCT_RULES` via the deck's capabilities.
- **Membership** leads with the member's reason when there is one, highlights the row the `?from=` surface was about (a closed list, `src/lib/plus-surfaces.ts`), and tells Plus members "Plus active until <date>" — never "Renews", because Plus never renews automatically.
- **Switches** (`src/server/flags.ts`), each off unless exactly `"on"`, each hiding only its own new surface: `PLUS_DISCOVER_PROMPT` (Option A: the prompt appears only inside a settled empty deck state, never near a live card), `PLUS_UNDO_UI` (Free members see the existing Undo with a Plus tag; the server still refuses them), `PLUS_CHECKOUT_REMINDERS` (below), `PLUS_FUNNEL_ANALYTICS` (below).
- **Checkout reminder.** One `ACCOUNT_NOTICE` (`data.kind = "CHECKOUT_REMINDER"`) for an order awaiting payment for a day with no receipt, the member's newest, member without Plus; one per order ever and one per member per 30 days, under a per-member advisory lock; daytime in the Maldives only; no email, push only through the existing engine. It needs `PLUS_CHECKOUT_REMINDERS_SINCE` as well as the switch, and the cutoff can never be earlier than `CHECKOUT_REMINDER.notBefore`, which is after every order that existed when it was written.
- **Funnel analytics.** `PlusFunnelEvent` (own table, TEXT + CHECK rather than enums, RLS on, no policies): promotion seen / tapped from the browser via `/api/analytics/plus` (UUID key, known surface, active members, rate-limited, always 204), checkout / instructions / receipt / approval recorded by the server after its own write commits, keyed once per order. `recordPlusEvent` never throws. The admin Analytics page shows raw counts per surface and never a rate without its counts.
