...
14.8 Dating boundary. Community never creates Like, Match or Conversation rows and offers no message action. The author overlay is the read-only full profile via `getCommunityProfile`: visible when the author is ACTIVE and not blocked either way (contact blocks included); hidden age/location and the photo policy are applied; the Discover eligibility predicate is deliberately not applied, so Community does not reveal who is discoverable. No Like/Pass controls: a dating Like must go through the Phase 6 path and its caps (Free 30 / Plus 90). Invisible Mode: see §20 — pending the owner's decision, `COMMUNITY.invisibleModeParticipation = "ALLOWED"` keeps Community independent of Discover visibility (an Invisible Mode user can post; Community never feeds discovery). Switching it to `READ_ONLY` hides the composer and makes the server reject posts from Invisible Mode users.
...
| 9 Likes You + intros | Likes You grids (Free anonymised / Plus full), intros, Matches tab. |
...
- Invisible Mode and Community (Phase 8): the product definition (§12.6) makes Invisible Mode a Discover rule only and does not say whether an Invisible Mode user may post publicly in Community. Interim behaviour: allowed (`COMMUNITY.invisibleModeParticipation = "ALLOWED"`), because Community never feeds discovery and posts show only the public identity. The owner must confirm ALLOWED or choose READ_ONLY (composer hidden, posts rejected server-side); one constant flips it.
