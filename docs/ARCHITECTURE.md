...
28.5 No backdrop blur inside the shell. The first device test showed the sign-in screen paint and then die, leaving Chromium's "This page couldn't load" — its renderer-crash page, not a network error; the server logs showed the requests arriving and being answered 200 throughout. The cause is `backdrop-filter`: each element carrying one becomes a composited layer that must snapshot everything painted behind it, and the signed-out screen stacks five of them — both fields, both provider pills, the primary button — over a full-bleed cover photograph. Android's WebView renderer does not survive that on every device, while Chrome on the same phone, with its own process and GPU path, renders it without complaint. So the head script in `layout.tsx` sets `data-native="android"` on the document before first paint whenever the user agent carries the shell's suffix, and `globals.css` drops the blurs under that attribute, raising the surface opacities a little to keep the contrast the blur had been helping with. A browser never sets the attribute, so the website is untouched. `MainActivity` additionally survives a renderer death by rebuilding the activity around a fresh WebView, twice, before giving up and saying so — that is the general safety net; this is the specific cause.
