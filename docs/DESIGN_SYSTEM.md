...
| `--color-sand` / `--color-on-sand` | `#C6A962` / `#3B2F2A` | `#E0C27A` / `#1A1412` | Mellocrush Plus gold: Plus tags, plan badges, Premium labels, the Plus hero disc, the active-Plus card icon, and the Super Like star (a Plus act). Nothing else is gold |
...
| Round actions | Pass 56, Like 66, Super Like 44, View 48; full-profile Pass 60, Like 66, Super Like 52 |
| FAB | 54 |
...
- Drag right past 110 px likes, drag left passes, ← / → / ↑ work from the focused card, taps on the left/right 30 % step through photos, the centre tap and the View profile button open the full profile. The next card is the real card, fully rendered underneath under its own key with its photo loading eagerly, so promotion reuses the same DOM node and an already-decoded photo; the member's main photo after that is warmed in the cache, never the rest of the queue. Every card has an opaque base (`--card-base`) and shows its own 400 px variant until the full photo arrives, so the card beneath can never show through. A drag moves the card by writing compositor-only transforms per animation frame outside React; nothing re-renders until release.
...
- The ⭐ Super Like control sits after Like on the deck and in the full profile's floating row: a gold filled star on the quiet bordered circle, with the corner `PlusTag` when the member has no Plus (they still see it, and a tap explains Plus). The Report/Block buttons at the foot of the full profile arrive with Safety in Phase 10.
...
- **Likes You teaser** (Free): the count in the tab ("Likes You (1)"), one `LockedPhoto` tile per like drawn from four fixed decorative blurhashes (no data about anyone: no photo, blurhash, name or handle reaches the DOM or the payload), then a lock card: lock glyph in an aqua-soft circle, "Someone likes you ❤️" / "N people like you ❤️", "See who likes you", the Plus tag and the upgrade button. Plus sees real `ProfileCard` tiles (name, age, verified badge, location) that open the full profile with Like / Pass (Pass dismisses that like); liking back shows the match overlay and the Matches tab lists the conversation with "Say hello".
- **Sent** (every tier): the third segment of the Likes control ("Likes You (N) | Sent (N) | Matches (N)"). One line of explanation ("You liked them. If they like you back, it's a match."), then the same `ProfileCard` tile grid as Plus Likes You; a tile opens the full profile with no action buttons. Empty: "No pending likes." with Start swiping.
...
- **Super Like** (`src/components/features/discovery/super-like.tsx`, §12.20 of ARCHITECTURE): Free tap → `PlusLockSheet` "Super Like ⭐" / "Stand out and send a message with your like." with one extra line "5 Super Likes every 7 days with Plus" (the count is read from `PRODUCT_RULES`, so it cannot drift). Plus tap → `SuperLikeComposer`, a `ResponsiveDialog`: gold star disc, "Send a Super Like ⭐" / "Stand out and say something first to <name>.", an optional `Textarea` (16 px, so iOS never zooms), a right-aligned "n / 150" counter that turns danger-red past the limit (and disables Send), a full-width coral "Send Super Like" with the star, and the allowance line "⭐ 3 of 5 Super Likes left · Resets in 4 days". The text resets on every opening. With none left, `SuperLikesUsedDialog`: "You've used your 5 Super Likes" / "Your Super Likes reset in 2 days." and one OK — never an upgrade. `SuperLikeBadge` is the small gold "⭐ Super Like" pill on tiles (a 2 px gold ring marks the tile); `SuperLikeNote` is the glass card at the top of a full profile with the label and the quoted message, always rendered as text. In chat the delivered message carries a small "⭐ Super Like" caption inside its bubble. Free Likes You: a gold-ringed row "⭐ Someone Super Liked you" / "They sent you a message" above the tiles, and the first N locked tiles starred — nothing about who.
