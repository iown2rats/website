 ...
 *
 * ONE ADDITION since then, on 22 September 2026: clause 26 gained a description of the first-party website
 * analytics added in ARCHITECTURE §30, because that feature measures visitors and therefore has to be disclosed
 * before it runs. The addition follows the rule above rather than bending it — every line of it describes
 * processing the code actually performs, and the list of what is NOT collected is enforced by
 * src/lib/analytics/classify.ts and by the absence of the corresponding columns in the schema, not by intention.
 */
    ...
    <>
      ...
      >
        ...
        <Clause n={9} title="Technical Information">
          ...
          <P>
            IP addresses are processed by MelloCrush&rsquo;s hosting, security and infrastructure providers in the ordinary course of delivering the Service
            &mdash; for example to route requests, prevent abuse, mitigate attacks, enforce rate limits and maintain operational and security logs. Those
            providers may process and retain IP addresses under their own arrangements and retention periods.
          </P>
          <P>
            That infrastructure and security processing is separate from the first-party website analytics described in clause 26. MelloCrush&rsquo;s website
            analytics does not receive or store IP addresses, and no IP address is written to the analytics records described in that clause.
          </P>
        </Clause>
        ...
        <Clause n={26} title="Cookies and Similar Technologies">
          ...
          <P>
            MelloCrush operates its own website analytics. It is first-party: the measurement is performed by MelloCrush on its own servers, and analytics data
            is not sent to an external analytics provider or advertising network. Two cookies support it — one identifying a browser so that repeat visits can be
            distinguished from first visits, and one identifying a single visit, which expires after 30 minutes of inactivity. Both contain only a random
            reference generated by MelloCrush, are marked HttpOnly so page scripts cannot read them, and are not linked to any advertising profile.
          </P>
          <P>For each page view, MelloCrush records:</P>
          <Bullets
            items={[
              "the random browser and visit references described above;",
              "the page path, with identifiers removed — a conversation is recorded as /chats/:id, never as the individual conversation;",
              "a general device type (mobile, tablet or desktop);",
              "a browser family and operating-system family, such as “Chrome” and “Android”, without version numbers;",
              "a two-letter country code, where the hosting provider supplies one;",
              "the website that linked to MelloCrush, recorded as its hostname only and not the full referring URL or page; and",
              "the account identifier, only where the visit carries a valid signed-in session.",
            ]}
          />
          <P>For the avoidance of doubt, MelloCrush&rsquo;s website analytics does NOT collect or store:</P>
          <Bullets
            items={[
              "IP addresses;",
              "full browser User-Agent strings;",
              "GPS, precise or device-reported location;",
              "device fingerprints, or any measurement of screen, fonts, hardware or timing intended to single out a device;",
              "query strings or link fragments, which is where search terms and personal details usually appear; or",
              "any attempt to identify a visitor who is not signed in.",
            ]}
          />
          <P>
            A visitor who is not signed in remains an anonymous reference. MelloCrush does not attempt to match that reference to a person, and staff activity in
            the administration area is excluded from these measurements. Raw analytics events are deleted after 90 days.
          </P>
        </Clause>
