    ...
    <AdminPage title="Audit log" description="Every privileged action, newest first. Entries are never edited or deleted from the dashboard.">
      ...
      {sp.targetId || sp.actorId ? (
        <p className="text-caption text-text-secondary">
          Filtered to {sp.targetId ? `target ${sp.targetId}` : ""}{sp.actorId ? `actor ${sp.actorId}` : ""}. <Link href="/admin/audit" className="font-medium text-primary-ink hover:underline">Clear</Link>
        </p>
      ) : null}
        ...
        {result.items.map((e) => (
          <div key={e.id} className="flex flex-col gap-1.5 border-b border-border px-4 py-3 last:border-0">
            <div className="flex flex-wrap items-center gap-2 text-caption">
              ...
              <span className="text-text-secondary">by {e.actor ? <Link href={`/admin/users/${e.actor.userId}`} className="font-medium text-primary-ink hover:underline">{e.actor.displayName ?? e.actor.userId}</Link> : "system"}</span>
              {e.targetType ? (
