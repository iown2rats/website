    ...
    <AdminPage title={o.reference} description={`${o.planName} · ${o.amountLabel}`} backHref={{ href: "/admin/payments", label: "Payments" }} actions={<StatusPill status={o.status} />}>
      ...
      <div className="grid grid-cols-1 gap-4 desktop:grid-cols-2">
        ...
        <Panel title="Customer and payment method">
          <KeyValueList
            items={[
              { label: "Customer", value: <Link href={`/admin/users/${o.user.userId}`} className="font-medium text-primary-ink hover:underline">{o.user.displayName ?? "(no name)"}{o.user.handle ? ` @${o.user.handle}` : ""}</Link> },
