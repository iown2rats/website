...
import { describeSignInIdentity } from "@/server/auth/identity";
import { PROVIDER_LABEL } from "@/server/auth/oidc";
import { ROUTES } from "@/server/auth/route-access";

export const metadata = { title: "Account deleted" };

/**
 * The Google or Telegram account that just signed in belongs to a Mellocrush account that was deleted (docs/ARCHITECTURE.md §4.4).
...
export default async function DeletedAccountPage() {
  ...
  const who = describeSignInIdentity(pending);
