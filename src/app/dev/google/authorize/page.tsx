    ...
    <main className="min-h-dvh bg-[#f8f9fa] px-4 py-8 text-[#1f1f1f]">
      <div className="mx-auto flex w-full max-w-[420px] flex-col gap-5 rounded-2xl border border-[#dadce0] bg-white p-7 shadow-sm">
        <div className="flex flex-col gap-1">
          <span className="inline-flex w-fit items-center rounded-full bg-amber-100 px-2.5 py-0.5 text-[11px] font-medium uppercase tracking-wide text-amber-800">Development identity provider</span>
          <h1 className="text-[22px] font-medium">Choose an account</h1>
          <p className="text-[14px] text-[#5f6368]">to continue to <span className="font-medium text-[#1f1f1f]">Mellocrush (local)</span>. This screen stands in for {label} and does not exist in production.</p>
        </div>
              ...
              <li key={i.providerSubject}>
                <form method="post" action="/dev/google/issue" className="m-0">
                  ...
                  <button type="submit" className={`flex w-full items-center gap-3 border-0 bg-transparent px-4 py-3 text-left hover:bg-[#f1f3f4] ${hint && hint === i.email ? "bg-[#e8f0fe]" : ""}`}>
                    <span className="grid size-9 shrink-0 place-items-center rounded-full text-[14px] font-medium text-white" style={{ background: accent }}>{(name[0] ?? "?").toUpperCase()}</span>
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-[14px] font-medium">{name}{deleted ? " · previous account deleted" : ""}</span>
        ...
        <form method="post" action="/dev/google/issue" className="m-0 flex flex-col gap-2.5 rounded-xl border border-dashed border-[#dadce0] p-4">
          {hidden}
          <p className="m-0 text-[13px] font-medium">Use another account</p>
          {telegram ? (
          ...
          <button type="submit" className="h-10 rounded-lg border-0 text-[14px] font-medium text-white" style={{ background: accent }}>Continue</button>
        ...
        <a href={cancelHref} className="text-center text-[13px] font-medium" style={{ color: accent }}>Cancel</a>
      </div>
    </main>
  );
}
