...
export function Wordmark({ height = 24, className, priority = false }: { height?: number; className?: string; priority?: boolean }) {
  const width = Math.round(height * WORDMARK_ASPECT);
  return <Image src={WORDMARK_SRC} alt={BRAND_NAME} width={width} height={height} priority={priority} unoptimized className={cn("block shrink-0 select-none", className)} style={{ width, height }} />;
}
