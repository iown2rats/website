...
export function VerificationDecision({ userId, hasSelfie }: { userId: string; hasSelfie: boolean }) {
  ...
  return (
    <div className="flex gap-2">
      ...
      <ConfirmationDialog open={approveOpen} onClose={() => setApproveOpen(false)} onConfirm={() => void decide("VERIFIED")} loading={busy} title="Mark as verified?" description="Only after comparing the selfie with the profile photos. The badge appears on their profile immediately." confirmLabel="Verify" />
