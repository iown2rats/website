...
export function BoostControl({ boost, tier, serverTime, onSync, onBoosted, onLocked }: { boost: BoostDto; tier: "FREE" | "PLUS"; serverTime: () => number; onSync: (iso: string) => void; onBoosted: (next: BoostDto) => void; onLocked: () => void }) {
  ...
  if (active) {
    ...
    return (
      <span className="inline-flex h-11 shrink-0 items-center gap-1 whitespace-nowrap rounded-md bg-ocean px-2.5 text-caption font-bold text-on-ocean tabular-nums" role="status" aria-live="polite" aria-label={`Boost active, ${formatDuration(activeMs)} left`}>
