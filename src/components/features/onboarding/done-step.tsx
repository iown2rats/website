...
const MISSING_LINKS: Record<string, { label: string; slug: string }> = {
  ...
  interestedIn: { label: "who you'd like to meet", slug: STAGE_META.MEET.slug },
            ...
            {missing.map((m, i) => {
              ...
              return (
                <span key={m}>
                  ...
                  {link ? <Link href={`${ROUTES.onboarding}/${link.slug}`} className="font-semibold text-primary-pressed">{link.label}</Link> : m}
                </span>
              );
            })}
            .
