    ...
    <ResponsiveDialog open={open} onClose={onClose} labelledBy={titleId}>
      ...
      <div role="listbox" aria-label={title} className="flex shrink-0 flex-col overflow-hidden rounded-2xl border border-border bg-surface [&>*+*]:border-t [&>*+*]:border-border">
        ...
        {results.map((l) => {
          ...
          return (
            <button key={l.id} type="button" role="option" aria-selected={isSelected} onClick={() => choose(l.id)} className={cn("flex h-13.5 items-center justify-between border-0 bg-transparent px-4.5 text-left text-body font-semibold text-text hover:bg-surface-muted")}>
              <span>
                {l.name}
        ...
        {results.length === 0 ? <p className="px-4.5 py-4 text-body-sm text-text-secondary">No island or atoll matches that.</p> : null}
      </div>
