...
export function PreviewProfile({ card }: { card: DiscoveryCardDto | null }) {
    ...
    return (
      <div className="flex min-h-0 flex-1 flex-col justify-center">
        <EmptyState
          ...
          actions={<Link href="/profile/edit" className="inline-flex h-11 items-center rounded-lg bg-primary px-5 text-body-sm font-bold text-on-primary">Edit profile</Link>}
        />
      </div>
    );
  }
  return (
    <div className="relative flex min-h-0 flex-1 flex-col">
      ...
      <p role="status" className="pointer-events-none absolute inset-x-0 top-[calc(12px+var(--safe-top))] z-40 mx-auto w-fit rounded-full bg-ocean/90 px-4 py-2 text-caption font-semibold text-on-ocean shadow-lg">
