 ...
 */
const SCOPES: Record<DiscoveryFiltersDto["locationScope"], string> = { ANYWHERE: "Anywhere", GREATER_MALE: "Greater Malé", MY_ATOLL: "My atoll", SPECIFIC: "Specific island" };
  ...
  return (
    <PageOverlay title="Discovery preferences" backHref="/settings">
      ...
      <ListGroup>
        ...
        <ListRow asDiv label="Looking for" meta={filters.intent ? INTENT_LABELS[filters.intent] : "Any"} />
        <ListRow asDiv label={<span className="flex items-center gap-2">Height <PlusTag size="xs" /></span>} meta={filters.advancedEnabled ? (filters.heightMinCm || filters.heightMaxCm ? `${filters.heightMinCm ?? "…"}–${filters.heightMaxCm ?? "…"} cm` : "Any") : "Thundi Plus"} />
        <ListRow asDiv label={<span className="flex items-center gap-2">Education <PlusTag size="xs" /></span>} meta={filters.advancedEnabled ? (filters.education || "Any") : "Thundi Plus"} />
      </ListGroup>
