        ...
        {CARDS.map(([label, body], i) => {
          ...
          return (
            <div key={label} className="shrink-0 overflow-hidden rounded-[22px] glass-card">
              <button type="button" aria-expanded={isOpen} aria-controls={`safety-${i}`} onClick={() => setOpen(isOpen ? null : i)} className="flex w-full items-center gap-3.5 border-0 bg-transparent px-3.5 py-3 text-left text-text">
                <span className="grid size-10 shrink-0 place-items-center rounded-[14px] bg-aqua-soft text-body-sm font-medium text-ocean">{i + 1}</span>
                <span className="flex-1 text-body-lg font-medium">{label}</span>
                <ChevronDownIcon size={18} className={cn("shrink-0 text-text-secondary transition-transform duration-200", isOpen && "rotate-180")} />
              </button>
              {isOpen ? <p id={`safety-${i}`} className="m-0 px-3.5 pb-3.5 pl-18 text-body-sm leading-relaxed text-text-secondary">{body}</p> : null}
            </div>
          );
        })}
      </div>
