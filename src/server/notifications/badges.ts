...
/** Unread counts for the bottom nav / sidebar. Likes = unread like/intro notifications; Chats = conversations with unread messages. */
export async function getNavBadges(actor: Actor, deps: { db?: Db } = {}): Promise<NavBadgeCounts> {
  ...
  const [likes, chats] = await Promise.all([
    ...
    db.notification.count({ where: { userId: actor.userId, readAt: null, type: "MESSAGE" } }),
  ]);
  return { likes, chats };
}
