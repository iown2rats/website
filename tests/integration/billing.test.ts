...
describe("unchanged foundations", () => {
  it("Provider-keyed authentication (Google, Telegram and Email), contact blocking and Invisible Mode + Community semantics are untouched", async () => {
    const providers = await db.$queryRaw<{ v: string }[]>`SELECT unnest(enum_range(NULL::"AuthProvider"))::text AS v`;
    expect(providers.map((p) => p.v)).toEqual(["GOOGLE", "TELEGRAM", "EMAIL"]);
